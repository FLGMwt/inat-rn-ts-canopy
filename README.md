...
- **Paste a branch, tag, or commit SHA** into the "Evaluate a ref" box to fetch that ref live from GitHub instead, or link directly with `?branch=<branch>`, `?ref=<tag-or-other-ref>`, or `?sha=<full-sha>` (subject to the API's public rate limit of 60 requests/hour without auth).
